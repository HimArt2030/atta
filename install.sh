...
pip3 install flask flask-socketio requests beautifulsoup4 fpdf2 colorama dnspython 2>/dev/null
if [ $? -eq 0 ]; then
    echo -e "${GREEN}    [+] تم تثبيت جميع المكتبات بنجاح${NC}"
else
    echo -e "${YELLOW}    [!] جرب: pip3 install --break-system-packages flask flask-socketio requests beautifulsoup4 fpdf2 colorama dnspython${NC}"
    pip3 install --break-system-packages flask flask-socketio requests beautifulsoup4 fpdf2 colorama dnspython 2>/dev/null
fi
